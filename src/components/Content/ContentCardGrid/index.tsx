...
export type ContentCardGridItem = {
  id?: string
  title?: string
  description?: string
    ...
    <div className={cn('w-full', className)}>
      <div className={cn('grid', gridCols[columns], gapClasses[gap])}>
        {items.map((item, index) => (
          <Card key={item.id || index} className="h-full flex flex-col hover:shadow-lg transition-shadow">
            {item.image && (
            ...
            {item.content && <CardContent className="flex-1">{item.content}</CardContent>}
