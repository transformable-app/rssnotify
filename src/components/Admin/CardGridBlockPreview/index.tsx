...
export type CardGridBlockPreviewProps = {
  items?: Array<{
    id?: string
    title?: string
    description?: string
...
export const CardGridBlockPreview: React.FC<CardGridBlockPreviewProps> = (props) => {
  ...
  if (!items || items.length === 0) {
    return (
      <Banner type="info">
        <p>No card items configured. Add items to see the preview.</p>
      </Banner>
    )
  }
  ...
  return (
    <div className="p-4">
      <Banner type="success" className="mb-4">
        <p>Card Grid Preview ({items.length} items, {columns} columns) - Width: {width === 'content' ? 'Content' : 'Full'}</p>
      </Banner>
      <div className={width === 'content' ? 'max-w-[1200px] mx-auto' : 'w-full'}>
        <ContentCardGrid items={cardItems} columns={columns} gap={gap} />
      </div>
    </div>
  )
}

export default CardGridBlockPreview
